...
def _import_class(module_and_class_name: str) -> type:
    ...
    module_name, class_name = module_and_class_name.rsplit(".", 1)  # ['text_recognizer.models', 'MLP']
...
def _setup_parser():
    ...
    # parser.add_argument("--CT_CL", default=True, action="store_true")
    parser.add_argument("--load_checkpoint", type=str, default=None)
    # biored
    parser.add_argument("--biored", default=True, action="store_true")
    # kg_prompt
    parser.add_argument("--use_kg_prompt", default=False, action="store_true")
    # entity_type
    parser.add_argument("--use_entity_type", default=False, action="store_true")
    # entity_loss
    parser.add_argument("--use_entity_loss", default=False, action="store_true")
    parser.add_argument("--e_lambda", type=float, default=0.01)
    # MLP
    parser.add_argument("--use_head", default=False, action="store_true")
    # Knowledge graph
    parser.add_argument("--use_kg", default=True, action="store_true")
    parser.add_argument("--use_kg_mlp", default=False, action="store_true")
    parser.add_argument("--align_lambda", type=float, default=0.01)
    parser.add_argument("--lp_lambda", type=float, default=0.01)
...
def _get_relation_embedding(data):
    ...
    for batch in tqdm(train_dataloader):
        with torch.no_grad():
            ...
            assert len(labels[0]) == len(relation_embedding)    # 36 
            for batch_idx, label in enumerate(labels.tolist()): # label 的形式是什么样的？ 几个token？
...
def main():
    ...
    data.tokenizer.save_pretrained('test')  # save the tokenizer
    ...
    if args.wandb:
        print("use wandb")
        logger = pl.loggers.WandbLogger(project="my_train", name=f"{dataset_name}")
    ...
    pl.callbacks.retain_graph = False
    ...
    step2_model_checkpoint = pl.callbacks.ModelCheckpoint(monitor="Eval/f1", mode="max",
        filename='{epoch}-Step2{Eval/f1:.2f}',
        dirpath="output",
        save_weights_only=True
    )
