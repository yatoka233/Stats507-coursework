...
def filter_prompts(h, t, id, max_len=5):
    templetes = json.load(open('/nfs/turbo/umms-drjieliu/proj/medlineKG/data/corpus/BioRED/output_prompt/tmp.json'))
    p = []
    ps = templetes[str(id)]
    for p_ in ps:
        if h in p_ and t in p_ and p_.index(h) < p_.index(t):
            p.append(p_)

    # if len(p) == 0:
    #     for p_ in ps:
    #         if h in p_ or t in p_:
    #             p.append(p_)
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, text_c=None, entity=None, sen_id=None):
...
class InputExampleWiki80(object):
    ...
    def __init__(self, guid, sentence, span1, span2, ner1, ner2, label, sen_id=None, nodes1=None, nodes2=None):
        self.guid = guid
        ...
        for example in dataset:
            ...
            if "id" in example:
                examples.append(InputExampleWiki80(guid=None,
                                ...
                                ner1=example['h']['type'] if 'type' in example['h'].keys() else None,
                                ner2=example['t']['type'] if 'type' in example['t'].keys() else None,
                                nodes1 = example['h']['mappings'],
                                nodes2 = example['t']['mappings'],
...
def convert_examples_to_features(examples, max_seq_length, tokenizer, args, rel2id):
    ...
    ## add external kg prompts
    if args.use_kg_prompt:
        kg_prompt_root = "/nfs/turbo/umms-drjieliu/proj/medlineKG/data/corpus/BioRED/output_prompt/id2templetes.json"
        kg_prompt_data = json.load(open(kg_prompt_root))

    if args.use_kg:
        # /nfs/turbo/umms-drjieliu/proj/prompt_re/kg_embeddings/data/node_index_mappings.json
        node2idx = json.load(open("/nfs/turbo/umms-drjieliu/proj/prompt_re/kg_embeddings/data/node_index_mappings.json"))
    ...
    if False:
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            assert s_type in type_list and o_type in type_list, (s_type, o_type)
            s_type = type_list.index(s_type) + 1
            o_type = type_list.index(o_type) + 1

            if mode.startswith("text"): # mode = 'text'
            ...
            if use_gpt:
                ...
                if args.use_template_words:
                    if args.use_entity_type:
                        prompt = f"What is the relation? [E{s_type}] {SUBJECT} [E{s_type}] {tokenizer.mask_token} [E{o_type}] {OBJECT} [E{o_type}] ."
                    else:
                        prompt = f"What is the relation? [sub] {SUBJECT} [sub] {tokenizer.mask_token} [obj] {OBJECT} [obj] ."
                        # prompt = f"What is the relation? {SUBJECT} {tokenizer.mask_token} {OBJECT} ."
                        # prompt = f"Predict the relation: [K] [P1] [sub] {SUBJECT} [sub] [P2] {tokenizer.mask_token} [P3] [obj] {OBJECT} [obj] [P4] [/K]"
            ...
            if args.use_kg_prompt:
                ...
                # kg_prompt = kg_prompt_data[str(sen_id)]
                kg_prompt = filter_prompts(SUBJECT, OBJECT, sen_id, max_len=5)
                prompt = prompt + " [SEP] Examples are " + kg_prompt
                # prompt = kg_prompt + " " + prompt

                

            if ex_index == 23:
                input_text = " ".join(tokens)
                # input_text = kg_prompt + " " + " ".join(tokens)
                logger.info(f"input text : {input_text}")
                logger.info(f"prompt : {prompt}")
                logger.info(f"label : {example.label}")
            ## order of prompt and tokens? default: prompt + tokens
            inputs = tokenizer(
                " ".join(tokens),
                # kg_prompt + " " + " ".join(tokens),
                prompt,
                truncation="longest_first",
                max_length=max_seq_length,
                padding="max_length",
            ...
            if False:
                ...
                assert sub_st != -1 and obj_st != -1 , (input_text, SUBJECT, OBJECT, inputs['input_ids'], SUBJECT_ids, OBJECT_ids)

            # find the subject and object tokens, choose the last ones
            if True:
                sub_st = sub_ed = obj_st = obj_ed = -1
                for i in range(len(inputs['input_ids'])):
                    if inputs['input_ids'][i:i+len(SUBJECT_ids)] == SUBJECT_ids:
                        sub_st = i
                        sub_ed = i + len(SUBJECT_ids)
                    if inputs['input_ids'][i:i+len(OBJECT_ids)] == OBJECT_ids:
                        obj_st = i
                        obj_ed = i + len(OBJECT_ids)
            ...
            x['ei'] = [s_type-1, o_type-1]

            if args.use_kg:
                x["nodes1"] = [node2idx[a] + 1 for a in example.nodes1] if len(example.nodes1) > 0 else [0]
                x["nodes2"] = [node2idx[a] + 1 for a in example.nodes2] if len(example.nodes2) > 0 else [0]
    ...
    ei = torch.tensor([o['ei'] for o in instances])
    if args.use_kg:
        nodes1 = torch.tensor([o['nodes1'] for o in instances])
        nodes2 = torch.tensor([o['nodes2'] for o in instances])
    ...
    if use_gpt:
        dataset = TensorDataset(input_ids, attention_mask, cls_idx, labels, so, ei)
    elif use_bert:
        if args.use_kg:
            dataset = TensorDataset(input_ids, attention_mask, token_type_ids, labels, so, ei, nodes1, nodes2)
        else:
            dataset = TensorDataset(input_ids, attention_mask, token_type_ids, labels, so, ei)
    else:
        dataset = TensorDataset(input_ids, attention_mask, labels, so, ei)
