...
class WIKI80(BaseDataModule):
    def __init__(self, args, model=None) -> None:
        ...
        # kg_prompt_tokens = [f"[P{i}]" for i in range(1,7)]
        # kg_prompt_tokens.extend(["[K]", "[/K]"])
        # self.tokenizer.add_special_tokens({'additional_special_tokens': kg_prompt_tokens})

        if args.use_entity_type:
            entity_type_tokens = [f"[E{i}]" for i in range(1,7)]
            self.tokenizer.add_special_tokens({'additional_special_tokens': entity_type_tokens})
