...
for epoch in range(n_epoch + 1):
    for input_nodes, positive_graph, negative_graph, blocks in dataloader:
        ...
        # loss = compute_loss_multilabel(pos_score[train_idx], neg_score[test_idx], positive_graph.edata['_TYPE'][train_idx])
        loss = compute_loss(pos_score[train_idx], neg_score[test_idx], device)
        # backward
        optimizer.zero_grad()
        loss.backward()
    ...
    if epoch % 5 == 0:
        ...
        pos_score = torch.where(pos_score>=thres, 1, 0)
        neg_score = torch.where(neg_score>=thres, 1, 0)
        # pos_label = positive_graph.edata['_TYPE']
        # neg_label = torch.tensor(positive_graph.edata['_TYPE'].max()+1).repeat(neg_score.shape[0])
        pos_label = torch.tensor(1).repeat(pos_score.shape[0])
        neg_label = torch.tensor(0).repeat(neg_score.shape[0])

        train_acc = calc_acc(torch.cat([pos_score[train_idx], neg_score[train_idx]]), torch.cat([pos_label[train_idx], neg_label[train_idx]]))
        if len(val_idx)>0:
        ...
        if len(test_idx)>0:
            ...
            # print('total num of negative guess in pos samples:', len(torch.where(pos_score==positive_graph.edata['_TYPE'].max()+1)[0]))
            # print('total num of negative guess in neg samples:', len(torch.where(neg_score==positive_graph.edata['_TYPE'].max()+1)[0]))
            print('total num of negative guess in pos samples:', len(torch.where(pos_score==0)[0]))
            print('total num of negative guess in neg samples:', len(torch.where(neg_score==0)[0]))
...
torch.save({
            ...
            'loss': loss,
            }, '/nfs/turbo/umms-drjieliu/proj/medlineKG/data/glkb_processed_data/openai_embeddings/dgl_graphs/glkb_hier.l2_binary_final_model.pth') # '/nfs/turbo/umms-drjieliu/proj/prompt_re/kg_embeddings/outputs/l2_binary_final_model.pth'
...
pickle.dump(dic, open('/nfs/turbo/umms-drjieliu/proj/medlineKG/data/glkb_processed_data/hgt_semantic_embeddings.pk', 'wb'))